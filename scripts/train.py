...
writer = SummaryWriter('syndet/res/res18')
...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, fname) in enumerate(train_loader):
        ...
        img_array2 = data.cpu().numpy()
        img_array2 = np.transpose(img_array2[0], (1, 2, 0))
        ...
        img_array0 = data_rot_v2.cpu().numpy()
        img_array0 = np.transpose(img_array0[0], (1, 2, 0))
        img_array0 = (img_array0 * 255).astype(np.uint8)
        img = Image.fromarray(img_array0)
        img.show()
        time.sleep(30)
        img.close()
        
        img_array1 = data_sft.cpu().numpy()
        img_array1 = np.transpose(img_array1[0], (1, 2, 0))
        ...
        loss_ang = b*loss_flp + loss_rot_v2
...
def test(args, model, device, test_loader):
    ...
    with torch.no_grad():
        for data, target, fname in test_loader:
            ...
            err_y += torch.sum(abs((output[:, 1] - target[:, 1]) * h)).item()
...
def main():
    ...
    parser.add_argument('--lr', type=float, default=5e-05, metavar='LR', help='learning rate (default: 1.0)')
    ...
    parser.add_argument('--a', type=float, default=0.05, metavar='M', help='rot ratio (default: 0.05)')
    parser.add_argument('--b', type=float, default=0.05, metavar='M', help='rot ratio (default: 0.05)')
    ...
    dataset1 = ImageNoLabDataset('data/pcbmo/origintrain/images', transform=transform1) # syndet/smtdata/images  
    # 数据集更换之后记得查看和更改图片读取格式(尤其是255- ！！)
    dataset2 = ImageWithObbDataset2('data/pcbmo/originval/images', 'data/pcbmo/originval/labels', transform=transform2)
    ...
    if args.save_model:
        weight_save_path='syndet/syndet_weight/syndet_res18_pcbmo_20240530.pth'
